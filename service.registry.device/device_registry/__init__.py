...
class IPSetter(Resource):
	def post(self):
		shelf = get_db()
		parser.add_argument('identifier', type=str, required=True, location="json")
		parser.add_argument('controller_gateway', type=str, required=True, location="json")
		args = parser.parse_args()
		new_device_id = args["identifier"]
		for k in list(shelf.keys()):
			if shelf[k]["identifier"].startswith(new_device_id) and not shelf[k]["controller_gateway"]:
				shelf[k]["controller_gateway"] = args["controller_gateway"]
				return {'message': 'Added controller_gateway', 'data':shelf[k]}, 201
		return {'message':'Bad identifier', 'data':''}, 400
...
api.add_resource(IPSetter, '/new_device')
